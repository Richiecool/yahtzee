...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    return this.freq(dice)[0] === 3 && this.freq(dice)[1] === 2 ? this.score : 0;
  };
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    return s.size === 4 && (!s.has(1,2) || !s.has(1,6) || !s.has(5,6)) ? this.score : 0;
...
const threeOfKind = new SumDistro({ count: 3, description:"Sum of all dices if 3 are the same" });
...
const smallStraight = new SmallStraight({ score: 30 , description:"30 poins for a small straight"});
